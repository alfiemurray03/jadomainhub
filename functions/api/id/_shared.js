...
export const FLOW_COOKIE = 'ja_domain_hub_oidc_flow';
export const SESSION_COOKIE = 'ja_domain_hub_id_session';
...
export function getOidcConfig(env) {
  ...
  const redirectUri = String(env.OIDC_REDIRECT_URI || 'https://jadomainhub.jagroupservices.co.uk/api/id/callback').trim();
...
function encodeText(value) { return base64UrlEncodeBytes(encoder.encode(value)); }
function decodeText(value) { return decoder.decode(base64UrlDecodeBytes(value)); }
...
export async function pkceChallenge(verifier) {
  return base64UrlEncodeBytes(new Uint8Array(await crypto.subtle.digest('SHA-256', encoder.encode(verifier))));
}
...
export async function verifySignedPayload(value, secret) {
  ...
  try {
    ...
    const valid = await crypto.subtle.verify('HMAC', key, base64UrlDecodeBytes(signature), encoder.encode(encoded));
    return valid ? JSON.parse(decodeText(encoded)) : null;
  } catch { return null; }
}

function decodeJwtPart(value) { return JSON.parse(decodeText(value)); }
...
export async function verifyIdToken(idToken, metadata, config, expectedNonce) {
  ...
  const keysResponse = await fetch(metadata.jwks_uri, { headers: { accept: 'application/json' }, cf: { cacheTtl: 300, cacheEverything: true } });
  ...
  const validSignature = await crypto.subtle.verify('RSASSA-PKCS1-v1_5', publicKey, base64UrlDecodeBytes(parts[2]), encoder.encode(`${parts[0]}.${parts[1]}`));
  const now = Math.floor(Date.now() / 1000);
  const audienceValid = claims.aud === config.clientId || (Array.isArray(claims.aud) && claims.aud.includes(config.clientId));
  const valid = validSignature && audienceValid && claims.iss === metadata.issuer
    && (!claims.tid || claims.tid === config.tenantId)
    && Number(claims.exp) > now
    && (!claims.nbf || Number(claims.nbf) <= now + 60)
    && claims.nonce === expectedNonce;
  if (!valid) throw new Error('Microsoft identity token validation failed');
  return claims;
}

export function normaliseUser(claims) {
  const email = String(claims.email || claims.preferred_username || (Array.isArray(claims.emails) ? claims.emails[0] : '') || '').trim();
