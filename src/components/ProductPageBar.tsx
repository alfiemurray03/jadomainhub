    ...
    <div className="border-b border-border bg-card/90 backdrop-blur-sm">
      <div className="mx-auto flex max-w-[1440px] flex-col gap-3 px-4 py-3 sm:px-6 md:flex-row md:items-center md:justify-between lg:px-8">
        <div className="min-w-0">
          <a href="/services" className="inline-flex min-h-8 items-center gap-1.5 text-xs font-semibold text-primary hover:underline">
        ...
        <Button asChild size="sm" className="min-h-11 w-full shrink-0 sm:w-auto md:self-auto">
          <a
            href={destination}
            {...(isExternal ? { target: '_blank', rel: 'noopener noreferrer' } : {})}
          >
            <span className="text-center">{isExternal ? 'View Plans on Storefront' : 'Request a Tailored Quote'}</span>
            {isExternal ? <ExternalLink className="ml-2 h-4 w-4 shrink-0" /> : <ArrowRight className="ml-2 h-4 w-4 shrink-0" />}
          </a>
        </Button>
      </div>
    </div>
